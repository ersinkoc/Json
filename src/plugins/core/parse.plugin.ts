import type { JsonKernel, JsonPlugin, JsonValue, ParseOptions, JsonResult } from '../../types';
import { JsonParseError, MaxDepthError } from '../../errors';
...
export function createParsePlugin(): JsonPlugin {
  return {
    ...
    install(kernel: JsonKernel) {
      const kernelImpl = kernel as unknown as { getConfig?: () => { parse?: ParseOptions }; call?: (name: string, ...args: unknown[]) => unknown };
       ...
       * ```
       */
      kernel.register('parse', (text: string, options?: ParseOptions): JsonValue => {
        const opts = { ...defaultOptions, ...options };
        const parser = new Parser(opts.maxDepth!, opts.maxLength!);
       ...
       * ```
       */
      kernel.register('safeParse', (text: string, options?: ParseOptions): JsonResult<JsonValue> => {
        try {
          const opts = { ...defaultOptions, ...options };
          const parser = new Parser(opts.maxDepth!, opts.maxLength!);
          let result = parser.parse(text);
