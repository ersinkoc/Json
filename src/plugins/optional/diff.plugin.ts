import type { JsonKernel, JsonPlugin, JsonPatchOperation, JsonValue } from '../../types';
...
function setValueByPointer(data: unknown, pointer: string, value: unknown): unknown {
  if (pointer === '' || pointer === '/') {
    return value;
  }
...
function generateDiff(
  ...
  if (before === undefined) {
    operations.push({ op: 'add', path: path || '/', value: after as JsonValue });
    return operations;
  }
  ...
  if (typeof before !== typeof after) {
    operations.push({ op: 'replace', path: path || '/', value: after as JsonValue });
    return operations;
  }
  ...
  if (isObject(before) && isObject(after)) {
    ...
    for (const key of allKeys) {
      ...
      if (!(key in before)) {
        operations.push({ op: 'add', path: keyPath, value: after[key] as JsonValue });
...
export function createDiffPlugin(): JsonPlugin {
  return {
    ...
    install(kernel: JsonKernel) {
       ...
       * ```
       */
      kernel.register('diff', (before: unknown, after: unknown): JsonPatchOperation[] => {
...
export function createPatchPlugin(): JsonPlugin {
  return {
    ...
    install(kernel: JsonKernel) {
       ...
       * ```
       */
      kernel.register('patch', (obj: unknown, operations: JsonPatchOperation[]): unknown => {
        let result = obj;
        ...
        for (const op of operations) {
          switch (op.op) {
            case 'add': {
              ...
              if (isArray(parent)) {
                const index = lastPart === '-' ? parent.length : parseInt(lastPart!.replace(/~1/g, '/').replace(/~0/g, '~'), 10);
                parent = [...parent.slice(0, index), op.value, ...parent.slice(index)];
                result = parentPath === '/' ? parent : setValueByPointer(result, parentPath, parent);
              } else if (isObject(parent)) {
                parent = { ...parent, [lastPart!.replace(/~1/g, '/').replace(/~0/g, '~')]: op.value };
                result = parentPath === '/' ? parent : setValueByPointer(result, parentPath, parent);
              } else {
                result = op.value;
              }
       ...
       * ```
       */
      kernel.register('validatePatch', (operations: JsonPatchOperation[]): { valid: boolean; errors?: string[] } => {
        const errors: string[] = [];
        ...
        for (let i = 0; i < operations.length; i++) {
          ...
          if ((op === 'add' || op === 'replace' || op === 'test') && !('value' in operation)) {
            errors.push(`Operation ${i}: missing "value" field for ${op} operation`);
          }

          if ((op === 'move' || op === 'copy') && !('from' in operation)) {
            errors.push(`Operation ${i}: missing "from" field for ${op} operation`);
          }
        }
       ...
       * ```
       */
      kernel.register('reversePatch', (operations: JsonPatchOperation[], original: unknown): JsonPatchOperation[] => {
        ...
        for (let i = operations.length - 1; i >= 0; i--) {
          ...
          if (opType === 'add') {
            ...
            reversed.push({ op: 'add', path: operation.path, value: originalValue as JsonValue });
          } else if (opType === 'replace') {
            const originalValue = getValueByPointer(original, operation.path);
            reversed.push({ op: 'replace', path: operation.path, value: originalValue as JsonValue });
          } else if (opType === 'move' && 'from' in operation) {
            reversed.push({ op: 'move', from: operation.path, path: operation.from });
          } else if (opType === 'copy') {
            reversed.push({ op: 'remove', path: operation.path });
          } else if (opType === 'test' && 'value' in operation) {
            reversed.push({ op: 'test', path: operation.path, value: operation.value });
          }
        }
